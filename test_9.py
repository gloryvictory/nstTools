...
SQL_QUERIES = {
    ...
    'get_layers': 'select t."Id"  as id, t."MapId" as map_id, t."Name" as name_ru, t."Url" as url, t."Type" as type from public."Layers" as t where t."Type" = \'xyz\' order by t."Name"',
    'insert_layer': """
...
def on_layer_select(sender, app_data, user_data):
    ...
    logger.info(f"Ткнули {selected_layers[panel_side]} ")
...
def move_layer_to_right():
    ...
    # Преобразуем selected_index в int и проверяем что он не None
    try:
        selected_index = int(selected_layers["left"]) if selected_layers["left"] is not None else None
        logger.info(f"Выбрали - индекс: {selected_index} ")
    ...
    try:
        with db_connection.cursor() as cur:
            ...
            if selected_index >= len(source_layers):
            ...
            logger.info(f"Выбрали: {selected_layer} ")
        ...
        with dpg.group(horizontal=True):
            ...
            with dpg.group(width=300):
                dpg.add_input_text(label="Пользователь", tag="username_input", default_value=DB_CONFIG['user'],
                                   width=250)
                dpg.add_input_text(label="Пароль", tag="password_input", default_value=DB_CONFIG['password'], password=True, width=250)
